...
def shirley_background_with_endpoints(x, y, i0=0, i1=-1, iters=50, tol=1e-5):
    """Shirley with configurable endpoints indices [i0, i1] inclusive (i1 may be -1)."""
    y = np.asarray(y); x = np.asarray(x)
    if i1 < 0:
        i1 = len(y) + i1
    y0, y1 = y[i0], y[i1]
    bg = np.linspace(y0, y1, i1 - i0 + 1)
    for _ in range(iters):
        # integrate (y - bg) over the subrange
        sub_x = x[i0:i1+1]
        sub_y = y[i0:i1+1]
        sub_bg = bg
        dx = np.diff(sub_x)
        mid = (sub_y[:-1] - sub_bg[:-1] + sub_y[1:] - sub_bg[1:]) * 0.5
        integral = np.empty_like(sub_y, dtype=float)
        integral[0] = 0.0
...
def shirley_background_adaptive(x, y, iters=50, tol=1e-5):
    """Compute Shirley; if corrected has negatives, set the min point as an endpoint and recompute once."""
    # initial
    ...
    if corr0[min_idx] >= 0:
        return bg0
    # Decide which side to treat as the fixed endpoint based on proximity to edges
    n = len(y)
    # If the minimum is closer to the right edge (high BE), make it the right endpoint; else left.
    if (n - 1 - min_idx) <= min_idx:
        # recompute using [0 .. min_idx] as the Shirley span; extend flat to the right
        bg1 = shirley_background_with_endpoints(x, y, i0=0, i1=min_idx, iters=iters, tol=tol)
    else:
        # recompute using [min_idx .. end] as the Shirley span; extend flat to the left
