...
`data/XPS_Si2p_siloxane_NG.csv` は意図的に選んだ難スペクトルです（Au電極上の薄いシロキサン層。Si 2p領域がAuゴースト線と重なり、SiO2ショルダーは急峻な背景上の微弱構造）。物理的に正しい分解（SiO2 102–103 eV / SiOx≒101 eV / ゴーストまたは元素状Si 99–100 eV）がBICで自動選択されます:
...
このケースは回帰テスト（`tests/test_benchmark_ng.py`）として固定されています。
